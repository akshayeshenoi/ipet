...
T = 1 #1 second
TICK = .1 # 100ms
CT_BIAS = 0.2112 # q from STP (0 means only mask when device is active, 1 means ILP (all the time))
...
def Perturb(ticks):
    # constants
    QUOTA_T = R * T # how much we can afford to spend over some T
    QUOTA_TICK = QUOTA_T / (T/TICK) # how much we can afford to spend over between ticks
    ...
    quota_t_left = QUOTA_T # 
    ct_start = -1

    def toggle_ct_active_on(tick_ts):
        nonlocal ct_active, ct_start, quota_t_left
        ct_active = True
        ct_start = tick_ts
        quota_t_left = QUOTA_T
    ...
    while True:
        ...
        for tick in ticks_extra:
            # initialize total quota for this tick
            ...
            quota_tick = QUOTA_TICK

            # first send any buffered packet
            buffered_user_activity_cpy = buffered_user_activity[:]
            for i in range(len(buffered_user_activity_cpy)):
                buff_packet = buffered_user_activity_cpy[i] 
                if buff_packet[0] <= quota_tick:
                    # can fit entirely
                    packet_to_send = buff_packet[0]
                    direction = buff_packet[1]
                    send(packet_to_send, tick, direction, ct=False)
                    # update quota
                    quota_tick -= packet_to_send
