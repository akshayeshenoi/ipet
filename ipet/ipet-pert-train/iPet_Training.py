...
iot_fingerprint_dir = os.path.join(parent, "IoT_Device_Fingerprinting")
combined_main_features = np.load(iot_fingerprint_dir + '/data/Traces_Mseq/new_features_main.npy')
combined_silences = np.load(iot_fingerprint_dir + '/data/Traces_Mseq/new_silences.npy')
combined_labels = np.load(iot_fingerprint_dir + '/data/Traces_Mseq/new_labels.npy')
...
print("Preparing for iPET Training ..... ")
...
def train(max_packets,max_len, n_epochs=5):
    devices = [device for device in np.unique(combined_labels)]
    discriminator = tf.keras.models.load_model(iot_fingerprint_dir+'/Models/M_seq_fingeprinting')  
    print("Starting iPET Training.....") 
    gans = dict()
    ...
    for i in range(n_epochs):          
        ...
        for device in devices:
            ...
            gans[device].fit([Noise,main_features,silences],labels,epochs = ep)
        ...
        discriminator.fit([final_main_features,final_silences],final_labels,epochs = er)
...
train(max_packets_per_omega,max_payload_per_omega,training_stages)